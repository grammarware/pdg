...
public CF ifCF(Statement stat) {
	...
	if(isBreak(stat.thenBranch) || isContinue(stat.thenBranch)) {
		condFollowdByBC += cond;
	}
	
	if(Statement::\if(_,_) := stat) {
		lastStatements += cond;
	} else {
...
public CF returnCF(Statement stat) {
	...
	if(Statement::\return(Expression expression) := stat) {
		uses[counting] = extractUse(stat.expression);
	}
...
public default CF concatCF(CF mainCF, list[Statement] restStatements) {
	...
	if(firstCF.firstStatement == -2 || firstCF.firstStatement == -3) {
		for(l <- mainCF.lastStatements) {
			if(l notin breakOrContinue[loop]) {
	...
	if(firstCF.firstStatement == -1 && size(restStatements) > 1) {
		return concatCF(mainCF, tail(restStatements));
	}
