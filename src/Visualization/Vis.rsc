...
private Figure graphs(loc project, int methNum){
	...
	row1 = [ecliCFG, ecliPDT];
	row2 = [ecliCDG, ecliDDG];
	
	return box(grid([row1, row2], lineColor("white"), vgap(10), hgap(50)), hsize(600), hresizable(false), lineColor("white"));
}

private bool(int button, map[KeyModifier,bool] modifiers) openGraph(loc project, int methNum, str graphType) =
